...
const QueryGenerator = {
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    let uniqueKeys = Object.keys(rawAttributes).filter(e => (rawAttributes[e].unique === true));
    if ((uniqueKeys.length > 1 && (!options.conflict || (options.conflict && !options.conflict.constraint))) ||
    ...
    if (options.conflict.update === false) {
      ...
      if (!options.conflict.update) {
        options.conflict.update = Object.keys(updateValues)
          .filter(value => value !== options.conflict.target);
      }

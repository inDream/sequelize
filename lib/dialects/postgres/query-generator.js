...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    ...
    if (options.conflict.update.columns === false) {
      ...
      console.log('pgUpsert_QG : 1.) options.conflict.update.columns = ' + JSON.stringify(options.conflict.update.columns))
      ...
      console.log('pgUpsert_QG: 2.) options.conflict.update.columns = ' + JSON.stringify(options.conflict.update.columns))
    ...
    console.log('pgUpsert_QG: 3.) options.onConflict = ' + JSON.stringify(options.onConflict))

    options.returning = '*, (xmax = 0)::bool AS xmax';

    // console.log('pgUpsert_QG: 4.) options = ' + JSON.stringify(options));

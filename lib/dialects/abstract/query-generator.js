    ...
    if (this.dialect === 'postgres' && this._dialect.supports['ON CONFLICT']) {
      if (options.onConflict && semver.gte(this.sequelize.options.databaseVersion, '9.5.0')) {
        ...
        if (constraint) {
          ...
          emptyQuery += ' ON CONFLICT (' + conflictTarget + ') ' + options.onConflict;
        }
      } else {
  ...
  */
  bulkInsertQuery(tableName, attrValueHashes, options, rawAttributes) {
    ...
    if (this._dialect.supports.updateOnConflict && options.updateOnConflict) {
      ...
      let conflict = '';
      const constraint = options.updateOnConflict.constraint&&this.quoteIdentifier(options.updateOnConflict.constraint);
      const target = options.updateOnConflict.target&&this.quoteIdentifier(options.updateOnConflict.target);
      if (constraint) {

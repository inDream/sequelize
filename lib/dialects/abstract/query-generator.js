...
const QueryGenerator = {
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this.dialect === 'postgres' && this._dialect.supports['ON CONFLICT']) {
      if (options.onConflict && semver.gte(this.sequelize.options.databaseVersion, '9.5.0')) {
        let constraint = options.conflict.constraint;
        let target = options.conflict.target;
        if (constraint) {
          valueQuery += ' ON CONFLICT ON CONSTRAINT ' + this.quoteIdentifier(constraint) + options.onConflict;
          emptyQuery += ' ON CONFLICT ON CONSTRAINT ' + this.quoteIdentifier(constraint) + options.onConflict;
        } else if (target) {
          valueQuery += ' ON CONFLICT (' + this.quoteIdentifier(target) + ') ' + options.onConflict;
          emptyQuery += ' ON CONFLICT (' + this.quoteIdentifier(target) + ') ' + options.onConflict;
        }
      } else {
  ...
  */
  bulkInsertQuery(tableName, attrValueHashes, options, rawAttributes) {
    ...
    if (this._dialect.supports.updateOnConflict && options.updateOnConflict) {
      ...
      let conflict = '';
      let constraint = this.quoteIdentifier(options.updateOnConflict.constraint);
      let target = this.quoteIdentifier(options.updateOnConflict.target);
      if (constraint) {
        conflict = ' ON CONFLICT ON CONSTRAINT ' + target;
      } else {
        conflict = ' ON CONFLICT (' + target + ')';
      }

      if (options.updateOnConflict.update === false) {
        onConflictKeyUpdate += ' DO NOTHING ';
      } else {
        onConflictKeyUpdate += conflict + ' DO UPDATE SET ' + options.updateOnConflict.update.map(attr => {
          const field = rawAttributes && rawAttributes[attr] && rawAttributes[attr].field || attr;
          const key = this.quoteIdentifier(field);

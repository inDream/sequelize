  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this._dialect.supports['ON CONFLICT'] && options.onConflict) {
      valueQuery += ' ON CONFLICT (' + this.quoteIdentifier(options.conflict.target) + ') ' + options.onConflict;
      emptyQuery += ' ON CONFLICT (' + this.quoteIdentifier(options.conflict.target) + ') ' + options.onConflict;
    }
  ...
  */
  bulkInsertQuery(tableName, attrValueHashes, options, rawAttributes) {
    ...
    if (this._dialect.supports.updateOnConflict && options.updateOnConflict) {
      ...
      console.log(options.updateOnConflict);

      let target = this.quoteIdentifier(options.updateOnConflict.target);
      onConflictKeyUpdate += ' ON CONFLICT (' + target + ') DO UPDATE SET ' + options.updateOnConflict.update.map(attr => {
        const field = rawAttributes && rawAttributes[attr] && rawAttributes[attr].field || attr;
        const key = this.quoteIdentifier(field);
